...
winding_t *ClipToSeperators(winding_t *source, winding_t *pass, winding_t *target, qboolean flipclip, pstack_t *stack)
{
  ...
  for (i = 0; i < source->numpoints; i++) {
    ...
    for (j = 0; j < pass->numpoints; j++) {
      ...
      length =
          plane.normal[0] * plane.normal[0] + plane.normal[1] * plane.normal[1] + plane.normal[2] * plane.normal[2];
      ...
      for (k = 0; k < source->numpoints; k++) {
        ...
        if (d < -ON_EPSILON) { // source is on the negative side, so we want all
          ...
          fliptest = false;
          break;
        } else if (d > ON_EPSILON) { // source is on the positive side, so we
                                     // want all
