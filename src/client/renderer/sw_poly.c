        ...
        if (count) {
          ...
          if (!iswater) {
            if (snext > bbextents)
              ...
              snext = AFFINE_SPANLET_SIZE; // prevent round-off error on <0
                                           // steps from
            ...
            if (tnext > bbextentt)
              ...
              tnext = AFFINE_SPANLET_SIZE; // guard against round-off error on
                                           // <0 steps
          }
...
void R_DrawAlphaSurfaces(void)
{
  ...
  while (s) {
    ...
    //			R_ClipAndDrawPoly( 0.60f, ( s->texinfo->flags & SURF_WARP) !=
    // 0, true ); 		else 			R_ClipAndDrawPoly(
    // 0.30f, ( s->texinfo->flags & SURF_WARP) != 0, true );
