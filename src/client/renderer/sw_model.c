...
void Mod_LoadVertexes(lump_t *l)
{
  ...
  out = Hunk_Alloc((count + 8) * sizeof(*out));
...
void Mod_LoadBrushModel(model_t *mod, void *buffer)
{
  ...
  lump_size = sizeof(dheader_t) / 4;

  for (i = 0; i < lump_size; i++)
...
void Mod_LoadAliasModel(model_t *mod, void *buffer)
{
  ...
  pinmodel_size = sizeof(dmdl_t) / 4;

  // byte swap the header fields and sanity check
  for (i = 0; i < pinmodel_size; i++)
...
void Mod_LoadSpriteModel(model_t *mod, void *buffer)
{
  ...
  if (sprout->numframes > MAX_MD2SKINS) {
    Sys_Error("%s has too many frames (%i > %i)", mod->name, sprout->numframes, MAX_MD2SKINS);
  }
