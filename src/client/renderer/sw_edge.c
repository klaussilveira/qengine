...
/*
the complex cases add new polys on most lines, so dont optimize for keeping them
the same have multiple free span lists to try to get better coherence? low depth
complexity -- 1 to 3 or so
...
void R_BeginEdgeFrame(void)
{
  ...
  if ((r_refdef.vrectbottom - r_refdef.vrect.y) > 0) {
    memset(newedges + r_refdef.vrect.y, 0, (r_refdef.vrectbottom - r_refdef.vrect.y) * sizeof(edge_t *));
    memset(removeedges + r_refdef.vrect.y, 0, (r_refdef.vrectbottom - r_refdef.vrect.y) * sizeof(edge_t *));
  }
}
