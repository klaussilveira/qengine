...
void CM_InitBoxHull(void)
{
  ...
  if ((numnodes + 6 > MAX_MAP_NODES) || (numbrushes + 1 > MAX_MAP_BRUSHES) ||
      (numleafbrushes + 1 > MAX_MAP_LEAFBRUSHES) || (numbrushsides + 6 > MAX_MAP_BRUSHSIDES) ||
...
void CM_RecursiveHullCheck(int num, float p1f, float p2f, vec3_t p1, vec3_t p2)
{
  ...
  if (plane->type < 3) {
    ...
    if (trace_ispoint) {
      ...
      offset = (float) fabs(trace_extents[0] * plane->normal[0]) + (float) fabs(trace_extents[1] * plane->normal[1]) +
