...
#define MAX_STRING_TOKENS                                                                                              \
  80                        /* max tokens resulting from Cmd_TokenizeString                                            \
                             */
...
/*
 * DG: For some stupid reason, SV_WriteServerFile() and SV_ReadeServerFile()
 * used MAX_OSPATH as buffer length for CVAR_LATCH CVARS and saved the whole
 * buffer into $game/save/current/server.ssv, so changing MAX_OSPATH breaks
 * savegames... Unfortunately, for some other fucking reason MAX_OSPATH was 128
 * for non-Windows which is just horrible.. so I introduced
 * LATCH_CVAR_SAVELENGTH with the stupid values so I could bump MAX_OSPATH.
...
#define MONSTER_INFANTRY 1
#define MONSTER_SOLDIER_BLASTER 2
#define MONSTER_SOLDIER_MACHINEGUN 3
#define MONSTER_SOLDIER_SHOTGUN 4

extern vec3_t muzzle_flash_offset[];
