...
void InfantryMachineGun(edict_t *self)
{
  ...
  if (self->s.frame == FRAME_attak111) {
    ...
    G_ProjectSource(self->s.origin, muzzle_flash_offset[MONSTER_INFANTRY], forward, right, start);
    ...
    VectorSubtract(self->s.angles, aimangles[MONSTER_INFANTRY], vec);
    AngleVectors(vec, forward, NULL, NULL);
  }

  monster_fire_bullet(self, start, forward, 3, 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MONSTER_INFANTRY);
}
