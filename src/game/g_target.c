 ...
 */
void use_target_secret(edict_t *ent, edict_t *other, edict_t *activator /* acticator */)
{
  if (!ent) {
    return;
  }
 ...
 * trigger is touched/used, any trigger_crosslevel_target with the same trigger
 * number is automatically used when a level is started within the same unit. It
 * is OK to check multiple triggers. Message, delay, target, and killtarget also
 * work.
 */
 ...
 * YELLOW ORANGE FAT When triggered, fires a laser.  You can either set a target
 * or a direction.
 */
