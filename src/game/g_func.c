 ...
 * "lip"		override the default 4 pixel lip remaining at end of
 * move "health"	    if set, the button must be killed instead of touched
 * "sounds"
 *    1) silent
 *    2) steam metal
 *    3) wooden clunk
 ...
 * trigger field activates the door. "health"	    if set, door must be shot
 * open "speed"		movement speed (100 default)
 ...
 * brush will be the point around which it is rotated. It will rotate around the
 * Z axis by default.  You can check either the X_AXIS or Y_AXIS box to change
 * that.
 *
 ...
 *
 * "delay"			delay before first firing when turned on, default
 * is 0 "pausetime"		additional delay used only the very first time
 ...
 * "wait"		how long to hold in the open position (default 5, -1
 * means hold)
 */
