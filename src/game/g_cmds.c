...
void Cmd_Give_f(edict_t *ent)
{
  ...
  if (give_all || (Q_stricmp(Cmd_Argv(1), "health") == 0)) {
    if (Cmd_Argc() == 3) {
      ent->health = (int) strtol(Cmd_Argv(2), (char **) NULL, 10);
    } else {
  ...
  if (it->flags & IT_AMMO) {
    if (Cmd_Argc() == 3) {
      ent->client->pers.inventory[index] = (int) strtol(Cmd_Argv(2), (char **) NULL, 10);
    } else {
...
void Cmd_Kill_f(edict_t *ent)
{
  ...
  if (((level.time - ent->client->respawn_time) < 5) || (ent->client->resp.spectator)) {
    return;
  }
...
void Cmd_Wave_f(edict_t *ent)
{
  ...
  i = (int) strtol(Cmd_Argv(1), (char **) NULL, 10);
  ...
  *text = 0;
  ...
  for (i = 0, e2 = g_edicts + 1; i < maxclients->value; i++, e2++) {
    ...
    Com_sprintf(st, sizeof(st), "%02d:%02d %4d %3d %s%s\n", (level.framenum - e2->client->resp.enterframe) / 600,
                ((level.framenum - e2->client->resp.enterframe) % 600) / 10, e2->client->ping, e2->client->resp.score,
                e2->client->pers.netname, e2->client->resp.spectator ? " (spectator)" : "");
