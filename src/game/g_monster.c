...
qboolean monster_start(edict_t *self)
{
  ...
  if (self->monsterinfo.currentmove) {
    self->s.frame =
        ...
        (randk() % (self->monsterinfo.currentmove->lastframe - self->monsterinfo.currentmove->firstframe + 1));
  }
    ...
    while ((target = G_Find(target, FOFS(targetname), self->combattarget)) != NULL) {
      if (strcmp(target->classname, "point_combat") != 0) {
        PF_dprintf("%s at (%i %i %i) has a bad combattarget %s : %s at (%i %i %i)\n", self->classname,
                   ...
                   target->classname, (int) target->s.origin[0], (int) target->s.origin[1], (int) target->s.origin[2]);
      }
    }
  }
